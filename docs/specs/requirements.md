...
- The platform will contain an integrator function that will combine partial information on trains into a real-time message with planned times and real-time on all stops. When part of the data is not yet available, then the platform will give an estimation for the missing information. (See for details Match and merge ServiceRun data from multiple RUs)
  - The integration of data from different providers on one service run will provide consolidated delay information showing the same delay on coupled double tracktion service parts. If a part of a coupled service run is delayed this delay will along the route be indicated on the following stops (consolidated estimated time). 
  - The integration of data from different providers on one service run will provide consolidated delay information showing a reasonable delay across the route by correcting the delays to be ordered along the route with reasonable timings. The time beween two stops is not allowed to be below a reasonable time based on the planned time. If the travel time  is lower the indicated delay (consolidated estimated time) will be increased.
